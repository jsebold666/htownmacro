    ...
    if (currentX >= EVENTAREA_X_TOP_LEFT) and (currentX <= EVENTAREA_X_BOTTOM_RIGHT):
        if (currentY >= EVENTAREA_Y_TOP_LEFT) and (currentY <= EVENTAREA_Y_BOTTOM_RIGHT):
            ...
            if datetime.now() >= timer_warning_check + timedelta(seconds=WARNING_MSG_COOLDOWN):
                ...
                PlayWav(ALARM)
                send_discord_message(DISCORD_WEBHOOK_HTOWN,'**HALLOWEEN** | **[BOT:**] ->** move to entrance ! ' + GetName(FindItem()))
            return True
    else:
        return False
            ...
            for name in names_mobs:
                if GetName(FindItem()).lower().find(name.lower()) >= 0:
                    if FindItem() not in list_boss_alive:
                        ...
                        send_discord_message(DISCORD_WEBHOOK_HTOWN,'**HALLOWEEN NOTIFICATION**| * ' + '**Boss is UP:** ' + GetName(FindItem()) + " " + str(FindItem()) )
...
def survive(method):
    while True:
        if method == 'bands':
            ...
            if GetHP(Self()) < GetMaxHP(Self())*0.9:
                UseSkill('Spirit Speak')
                Wait(2000)
            elif IsPoisoned(Self()):
...
if __name__ == '__main__':
    ...
    survive_thread = threading.Thread(target=survive('necro'))
